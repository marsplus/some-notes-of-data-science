...
\end{equation*}
where the expectation is taken over all possible values of \( x \) weighted by their true probabilities under \( p(x) \). \highlight{This formulation assumes access to the entire underlying distribution}.
...
% Thus, updating the model parameters \( \theta \) to minimize cross-entropy is equivalent to maximizing the log-likelihood.

% \paragraph{Gradient of Cross-Entropy:}  
% Taking the gradient of the cross-entropy with respect to \( \theta \):

% \begin{equation*}
%    \nabla_{\theta} H(p, q) = -\mathbb{E}_{x \sim p} [\nabla_{\theta} \log q(x; \theta)].
% \end{equation*}


% \paragraph{Gradient of Log-Likelihood:}  
% Similarly, for the log-likelihood function:

% \begin{equation*}
%    \nabla_{\theta} \mathcal{L}(\theta) = \mathbb{E}_{x \sim p} [\nabla_{\theta} \log q(x; \theta)].
% \end{equation*}


% Minimizing the cross-entropy is exactly equivalent to maximizing the log-likelihood, up to a negative sign. This means that the optimization process for both objectives follows the same trajectory, with the difference being that one is a minimization problem while the other is a maximization problem.
...
\end{itemize}
    ...
    \subsubsection{A Derivation of Bias-Variance Trade-off}
        ...
        \subsubsection{An Example of Bias-Variance Decomposition for Linear Regression Models }
...
\subsection{Precision, Recall, and Other Measures}
    \begin{itemize}
...
A Markov network models the joint probability distribution of a set of variables using the \textit{Gibbs distribution}, which is defined as:
\[
...
To find the MLE estimate $\bm{\theta}^\ast$, we solve:
\[
...
\subsection{Principal Component Analysis (PCA)}
    \begin{itemize}
        ...
        % As $\bm{X}^\top \bm{X}$ is a real symmetric matrix, it can be diagonalized by an orthogonal matrix (i.e., $\bm{V}$).
...
\subsection{Regressions}
    ...
    \subsubsection{Linear Regression (LR)}\label{ML:linear-regression}
        \begin{itemize}
    ...
    \paragraph{Summary of Insights from the MAP Perspective.}
...
\subsection{Classification}
    ...
    \subsubsection{SVM}
        ...
        \end{itemize}
    \paragraph{A Quadratic Program (QP) Formulation of Linear SVM in Matrix Form.}
...
\subsection{Boosting}
    ...
    In particular, the approach optimizes a loss function $L(\cdot)$  by iteratively training the weaker learners (i.e., obtaining the model parameter $\theta_i$) and picking the associated weights $\beta_i$:
            ...
            \end{equation}
        % where $\SET{\theta_i}_{i=1}^{K}$ are model parameters of the learners and $\SET{\beta_i}_{i=1}^{K}$ the weights to combine the learners together.
